...
//#region node_modules/.nitro/vite/services/ssr/assets/auth-middleware-B3XILrvZ.js
...
var requireSupabaseAuth = createMiddleware({ type: "function" }).server(async ({ next }) => {
	...
	if (error || !userData?.user) throw new Error("Unauthorized: Invalid token");
	if (!userData.user.id) throw new Error("Unauthorized: No user ID found in token");
	return next({ context: {
		supabase,
