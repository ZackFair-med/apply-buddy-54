...
//#region node_modules/.nitro/vite/services/ssr/assets/jobs.functions-D4mvJpq4.js
...
var getJob = createServerFn({ method: "POST" }).validator((d) => objectType({ id: stringType().uuid() }).parse(d)).middleware([requireSupabaseAuth]).handler(getJob_createServerFn_handler, async ({ data, context }) => {
...
var createJob = createServerFn({ method: "POST" }).validator((d) => jobInputSchema.parse(d)).middleware([requireSupabaseAuth]).handler(createJob_createServerFn_handler, async ({ data, context }) => {
...
}).parse(d)).middleware([requireSupabaseAuth]).handler(updateJob_createServerFn_handler, async ({ data, context }) => {
...
var deleteJob = createServerFn({ method: "POST" }).validator((d) => objectType({ id: stringType().uuid() }).parse(d)).middleware([requireSupabaseAuth]).handler(deleteJob_createServerFn_handler, async ({ data, context }) => {
...
var deleteJobs = createServerFn({ method: "POST" }).validator((d) => objectType({ ids: arrayType(stringType().uuid()).min(1).max(500) }).parse(d)).middleware([requireSupabaseAuth]).handler(deleteJobs_createServerFn_handler, async ({ data, context }) => {
...
var updateJobsStatus = createServerFn({ method: "POST" }).validator((d) => objectType({
	...
	status: jobStatus
}).parse(d)).middleware([requireSupabaseAuth]).handler(updateJobsStatus_createServerFn_handler, async ({ data, context }) => {
